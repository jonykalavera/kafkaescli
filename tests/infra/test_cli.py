...
def consumer_command_mock(mocker):
    consumer_command = mocker.patch(
        "kafkaescli.app.commands.ConsumeCommand", autospec=True
    )
...
def test_consume_hello(consumer_command, mocker):
    ...
    consumer_command.assert_called_once_with(
        topics=topics, config=mocker.ANY, webhook=mocker.ANY, group_id=mocker.ANY
    )
